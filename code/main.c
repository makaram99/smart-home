...
/************************************************************************************
 * @brief Semaphore used by the ADC to indicate that the conversion is done. It is 
...
int main(void) {
    ...
    u8_AdcChannelToRead = system.sensors.gas.u8_AdcChannel;
...
void taskReadSensors(void * pv) {
	...
	while(1) {
        ...
        if(pdPASS == LOC_u8SemState) {
             ...
             *        system.sensors.gas.u8_AdcChannel, then swap the ADC channel
             *        to read the next sensor in the next iteration. 
             *************************************************************************/
            if(u8_AdcChannelToRead == system.sensors.gas.u8_AdcChannel) {
                system.sensors.gas.u16_CurrentValueInBinary = MADC_u16ADCRead();
                u8_AdcChannelToRead = system.sensors.temperature.u8_AdcChannel;
            } else if(u8_AdcChannelToRead == system.sensors.temperature.u8_AdcChannel) {
                system.sensors.temperature.u16_CurrentValueInBinary = MADC_u16ADCRead();
                u8_AdcChannelToRead = system.sensors.gas.u8_AdcChannel;
            } else {
                /* MISRA C */
            }
...
void taskControlGasLevel(void * pv) {
	while(1) {
        system.sensors.gas.u8_CurrentValue = (u8)(((u32)system.sensors.gas.u16_CurrentValueInBinary * system.sensors.gas.u8_MaxValue) / 1024.0f);

		if(system.sensors.gas.u8_CurrentValue >= system.sensors.gas.u8_CriticalValue) {
...
void taskControlTemperature(void * pv) {
	while(1) {
        system.sensors.temperature.u8_CurrentValue = (u8)((system.sensors.temperature.u16_CurrentValueInBinary * 500.0f) / 1024.0f);

        system.sensors.temperature.u8_CurrentValue = round(system.sensors.temperature.u8_CurrentValue);

        if(system.sensors.temperature.u8_CurrentValue > system.sensors.temperature.u8_CriticalValue) {
 ...
 * @note    This function writes to the LCD at the line 1, position 0, and uses
 ...
 * @note    This function writes to the LCD at the line 1, position 10, and uses
...
void displayTemperatureValue(void) {
    ...
    HCLCD_VidWriteNumber_4Bits(system.sensors.temperature.u8_CurrentValue);
    HCLCD_VidWriteString_4Bits((u8*)"C");
    
    /* Adding spaces to the LCD display to clear the previous value. */
    if(system.sensors.temperature.u8_CurrentValue < 10) {
			HCLCD_VidWriteString_4Bits((u8*)"  ");
	} else if(system.sensors.temperature.u8_CurrentValue < 100) {
		HCLCD_VidWriteString_4Bits((u8*)" ");
	}
}
 ...
 * @note    This function writes to the LCD at the line 2, position 0, and uses
 ...
 * @note    This function writes to the LCD at the line 2, position 6, and uses
 ...
 * @return  If the password is correct, return 1. Otherwise, return 0.
 *******************************************************************************/
